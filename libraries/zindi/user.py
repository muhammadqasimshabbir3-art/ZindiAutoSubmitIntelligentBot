...
class Zindian:
    ...
    def my_rank(self, challenge, user_name_for_rank=""):
    ...
    def get_available_remaining_submissions_for_competition(self, current_selected_challenge: str) -> dict:
        """Get remaining submissions for today for a competition."""
        competition_name = current_selected_challenge
        authen_dict = {"auth_token": self.__auth_data["auth_token"]}
        url = f"https://api.zindi.africa/v1/competitions/{competition_name}/submissions/limits"
        headers = {
            "accept": "*/*",
            "accept-encoding": "gzip, deflate, br, zstd",
            "accept-language": "en-US,en;q=0.9",
            "auth-token": f"{authen_dict['auth_token']}",
            "origin": "https://zindi.africa",
            "referer": f"https://zindi.africa/competitions/{competition_name}/submit",
        ...
        if response.status_code == 200:
            ...
            logger.error(f"ERROR API FAILED: {response.status_code}, {response.text}")
            return {}
            ...
            if n_sub > 0:
                if sb_time.shape[0] != 0:
                    ...
                    n_submitted_today = sb_time[
                        (sb_time.status.isin(["successful", "initial"])) & (sb_time.delta.dt.days < 1)
                    ].shape[0]
    ...
    @staticmethod
    def get_challenge_index_using_name(challenges_data, competition_name):
        """Get challenge index by competition name."""
        matching_indices = challenges_data[(challenges_data["id"] == competition_name)].index
        return matching_indices[0]

    def get_opened_challenges(
        self, reward="all", kind="competition", active="all", fixed_index=None, open_competition=True
    ):
        """Get opened competitions."""
    ...
    def select_a_challenge(
        ...
        if competition_name:
            challenge_index = self.get_challenge_index_using_name(challenges_data, competition_name)
    ...
    def download_dataset(self, destination=".", make_destination=True):
        ...
        if self.__challenge_selected:
            ...
            datafiles = []
            [datafiles.append(i) for i in datafiles_ if i not in datafiles]  # remove deplicates
    ...
    def get_leaderboard_data(self, user_name) -> list:
        ...
        for data in response:
            try:
                ...
                last_submission = (
                    "" if last_submission is None else pd.to_datetime(str(last_submission)).strftime("%d %B %Y, %H:%M")
                )
