...
def download(url="https://", filename="", headers=""):
    ...
    response = requests.get(url, headers=headers, data={"auth_token": headers["auth_token"]}, stream=True)
...
def print_lb(challengers_data, user_rank):
    ...
    for i in range(len(challengers_data)):
        ...
        try:
            score = (
                data["best_private_score"] if "best_private_score" in data else data["best_public_score"]
            )  # Best submission's score
            rank = data["private_rank"] if "private_rank" in data else data["public_rank"]  # Rank on leaderboard
            name = (
                data["user"]["username"] if "user" in data else f"TEAM - {data['team']['title']}"
...
def print_submission_board(submissions_data):
    ...
    for data in submissions_data:
        ...
        if data["status"] in ["successful", "initial"]:
            ...
            score = (
                data["private_score"] if "private_score" in data else data["public_score"]
...
def join_challenge(url, headers, code=False):
    ...
    if not code:
        response = requests.post(url=url, headers=headers, data={"auth_token": headers["auth_token"]})
    else:
